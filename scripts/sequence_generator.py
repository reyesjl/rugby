...
class SequenceGenerator:
    def __init__(self, index_path: Optional[str] = "./master_video_index.json"):
...
"""
        ...
        try:
            ...
            if content.startswith("```json"):
                # Remove leading ```json and trailing ``` precisely
    ...
    def _create_fallback_sequence(
        self, prompt: str, relevant_clips: list[dict]
    ) -> dict:
    ...
    for i, (name, config) in enumerate(demo_prompts.items(), 1):
        ...
        try:
            ...
            if "error" not in sequence:
                ...
                print(
                    f"   Duration: {sequence.get('total_estimated_duration', 'Unknown')}"
                )
    ...
    while True:
        ...
        try:
            ...
            for i, clip in enumerate(clips[:5], 1):  # Show first 5
                print(
                    f"   {i}. {clip.get('file', 'Unknown')} ({clip.get('start_time', '00:00')} - {clip.get('end_time', '00:00')})"
                )
            ...
            if save in ["y", "yes"]:
                ...
                filename = (
                    input(f"Enter filename [{default_name}]: ").strip() or default_name
                )
