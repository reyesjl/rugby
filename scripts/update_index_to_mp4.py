...
def update_index_to_mp4(
    index_file: str = os.path.join("animus", "data", "master_video_index.json"),
):
    ...
    for entry in index_data:
        ...
        if "tuesday_session_08_06_2025" in video_file and video_file.upper().endswith(
            ".MPG"
        ):
            tuesday_entries += 1

            # Check if corresponding MP4 exists
            mp4_path = video_file.replace(
                "tuesday_session_08_06_2025/", "tuesday_session_08_06_2025_mp4/"
            )
    ...
    if updates_made > 0:
        ...
        backup_file = os.path.join(
            os.path.dirname(index_file), "master_video_index_backup.json"
        )
...
if __name__ == "__main__":
    ...
    success = update_index_to_mp4(
        os.path.join("animus", "data", "master_video_index.json")
    )
