...
def build_parser() -> argparse.ArgumentParser:
    ...
    # index rebuild
    p_rebuild = sub_index.add_parser(
        "rebuild", help="Rebuild master index (MP4 preferred)"
    )
    p_rebuild.add_argument(
        "--summaries", default="./summaries", help="Summaries base dir"
    )
    p_rebuild.add_argument(
        "--transcripts", default="./transcripts", help="Transcripts base dir"
    )
    p_rebuild.add_argument(
        "--output",
        default=os.path.join("animus", "data", "master_video_index.json"),
        help="Output index path",
    )
    p_rebuild.add_argument(
        "--include-folders", nargs="+", help="List of folders to include"
    )
    p_rebuild.add_argument(
        "--no-prefer-mp4", action="store_true", help="Do not prefer MP4 when available"
    )
    p_rebuild.set_defaults(func=cmd_index_rebuild)

    # index mp4-only
    p_mp4 = sub_index.add_parser(
        "mp4-only", help="Filter current index to MP4-only (backs up full index)"
    )
    p_mp4.add_argument(
        "--index",
        default=os.path.join("animus", "data", "master_video_index.json"),
    ...
    # index update-to-mp4
    p_upd = sub_index.add_parser(
        "update-to-mp4", help="Update Tuesday session MPG → MP4 where available"
    )
    p_upd.add_argument(
        "--index",
        default=os.path.join("animus", "data", "master_video_index.json"),
